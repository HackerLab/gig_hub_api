...
describe GigsController do
  ...
  let(:valid_attributes) {{:company => "The Hacker's Guild", :location => "Hackramento", :commitment => "part-time", :name => "hacker", :description => "Lots of heroic hacking."}}
  ...
  context 'routing' do
    it {should route(:post, '/gigs').to :action => :create}
    it {should route(:get, '/gigs/1').to :action => :show, :id => 1}
    it {should route(:put, '/gigs/1').to :action => :update, :id => 1}
    it {should route(:delete, '/gigs/1').to :action => :destroy, :id => 1}
    it {should route(:get, '/gigs').to :action => :index}
  end
  ...
  context 'PUT update' do
    context 'with valid params' do
      it 'updates the requested gig' do
        ...
        Gig.any_instance.should_receive(:update_attributes).with({ 'these' => 'params' })
        put :update, {:id => gig.to_param, :gig => { 'these' => 'params' }}, valid_session
      end
    ...
    context 'with invalid params' do
      it 'assigns the gig as @gig' do
        ...
        # Trigger the behavior that occurs when invalid params are submitted
        Gig.any_instance.stub(:save).and_return(false)
        put :update, {:id => gig.to_param, :gig => {  }}, valid_session
        assigns(:gig).should eq(gig)
      end
